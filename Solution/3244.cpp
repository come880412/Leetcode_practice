...
public:
    vector<int> shortestDistanceAfterQueries(int n, vector<vector<int>>& queries) {
        ...
        for (auto const &query : queries) {
            ...
            auto it_start = s.lower_bound(fromCity + 1); // Find the value greater than or equal to fromCity + 1
            auto it_end = s.upper_bound(toCity - 1);     // Find the value greater than toCity - 1  
            s.erase(it_start, it_end); // Erase the value in range [it_start, it_end)
